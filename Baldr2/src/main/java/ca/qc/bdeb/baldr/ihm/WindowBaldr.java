...
public class WindowBaldr extends javax.swing.JFrame implements Observateur, Observer {
    ...
    private PanelTab tabSommaire = null;
    ...
    public WindowBaldr(Noyau noyau, GestionnaireFiltres filtre) {        
        ...
        setBounds((screen.width - frame.width) / 2,
    ...
    private void initComponents() {
        ...
        projectMenu.addMenuListener(new javax.swing.event.MenuListener() {
            public void menuCanceled(javax.swing.event.MenuEvent evt) {
            }
            public void menuDeselected(javax.swing.event.MenuEvent evt) {
            }
            public void menuSelected(javax.swing.event.MenuEvent evt) {
                projectMenuMenuSelected(evt);
            }
        });
...
/*    */
    ...
    private void ecrireDonneesDansTask() {
        for (PanelTab onglet : listeOnglets) {
            if (onglet != null) {
                ...
                //onglet.setFichiers(onglet.getPanelFile().getFileList(), false);//THREAD
            }
        }
    }
    ...
    private void projectMenuMenuSelected(javax.swing.event.MenuEvent evt) {//GEN-FIRST:event_projectMenuMenuSelected
        for (PanelTab onglet : listeOnglets) {
            if (onglet.isShowing() && onglet.estSommaire()) {
                ...
                if (listeOnglets.size() == 2) {
                    fermerTab(tabSommaire, false);
                }
            }
        }
    }//GEN-LAST:event_projectMenuMenuSelected
    ...
    public void fermerTab(PanelTab onglet, boolean toutSauf) {
        ...
        if (!toutSauf && !onglet.estSommaire()) {
...
//        projetCourant.retirerTask(onglet);//THREAD
    ...
    public void creerSommaire() {
        ...
        ajouterOnglet(messages.getString("Summary"), newTab);
    }

    /*
     Modifier le sommaire lorsqu'une analyse est ajoutée ou retirée
     */
    public void modifierSommaire() {
        boolean test = analyseSommaireOuverte;
        if (analyseSommaireOuverte) {

            // Fermture du sommaire hors-date
            fermerTab(tabSommaire, analyseSommaireOuverte);
     ...
     */
    public void ajustementSmartSommaire() {
        if (preferences.prefExists("SOMMAIRE")
                && (Boolean) preferences.readPref("SOMMAIRE", true)) {
     ...
     */
    private void ajouterOnglet(String titre, PanelTab newTab) {
        /**
    ...
    private boolean verifierSiLesPanelEstSommaire() {
            ...
            if (e.isPopupTrigger()
                ...
                for (PanelTab onglet : listeOnglets) {
                    if (onglet.isShowing() && onglet.estSommaire()) {
                        ...
                        menuRenommerAnalyse.setEnabled(false);
                        break;
                    } else {
                        menuFermerAnalyse.setEnabled(true);
                        menuFermerTout.setEnabled(true);
                        menuRenommerAnalyse.setEnabled(true);
                    }
                }
